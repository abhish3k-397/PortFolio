...
const StartPage = ({ onStart }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center overflow-hidden transition-colors duration-700 ${focusedPill === 'cyberpunk' ? 'bg-[#0a0000]' : focusedPill === 'inkpaper' ? 'bg-[#0f172a]' : 'bg-black'
            }`}>
            ...
            {step === 'choice' && (
                <div className="absolute inset-0 flex w-full h-full z-20">
                    ...
                    <div
                        className={`relative w-1/2 h-full flex items-center justify-center cursor-pointer transition-all duration-700 group ${focusedPill === 'inkpaper' ? 'bg-blue-500/5' : 'hover:bg-blue-900/5'
                            }`}
                        onClick={() => handleChoice('inkpaper')}
                        onMouseEnter={() => { setFocusedPill('inkpaper'); playHover(); }}
                        onMouseLeave={() => setFocusedPill(null)}
                    >
                        {/* Blue Atmosphere */}
                        <div className={`absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-cyan-900/20 via-slate-950 to-black transition-opacity duration-700 ${focusedPill === 'inkpaper' ? 'opacity-100' : 'opacity-0'
                            }`} />

                        {/* Blue Glow Effect */}
                        <div className={`absolute inset-0 bg-cyan-400/10 blur-[120px] transition-all duration-700 ${focusedPill === 'inkpaper' ? 'opacity-100 scale-125' : 'opacity-0 group-hover:opacity-30'
                            }`} />

                        {/* The Blue Pill */}
                        <div className={`relative w-16 h-32 md:w-24 md:h-48 rounded-full bg-gradient-to-b from-cyan-300 to-blue-600 shadow-[0_0_30px_rgba(34,211,238,0.3)] transition-all duration-500 z-10 ${focusedPill === 'inkpaper' ? 'scale-110 shadow-[0_0_120px_rgba(34,211,238,0.6)] -rotate-3' : 'scale-100 group-hover:scale-105 grayscale-[0.5] group-hover:grayscale-0'
                            }`}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
                        <div className="text-center transition-all duration-500">
                            ...
                            <p className={`text-sm md:text-xl mt-4 transition-all duration-500 ${focusedPill === 'cyberpunk'
                                ...
                                {focusedPill === 'cyberpunk' ? 'EMBRACE THE CHAOS' : focusedPill === 'inkpaper' ? 'FIND YOUR PATH' : 'YOUR DESTINY'}
                            </p>
                        </div>
                    </div>
                </div>
